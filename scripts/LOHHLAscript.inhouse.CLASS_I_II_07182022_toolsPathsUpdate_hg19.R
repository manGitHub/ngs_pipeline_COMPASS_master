...
.libPaths(c("/data/Compass/dev/valdezkm/LOHHLA/R_packages/R/4.2.0/library/", .libPaths()))
...
option_list = list(
  ...
  make_option(c("--samtools"), type="character", default='',
              help="path to Samtools executable [default= %default]", metavar="character"),
  make_option(c("--jellyfish"), type="character", default='',
              help="path to JellyFish executable [default= %default]", metavar="character"),
  make_option(c("-e", "--HLAexonLoc"), type="character", default='', 
  ...
  make_option(c("--coordinateFile"), type="character", default='', 
              help="table of loci coordinates to be evaluated [default= %default]", metavar="character"),
  make_option(c("--rLibraries"), type="character", default='',
              help="location of installed R libraries [default= %default]", metavar="character")
)
...
coordinateFile    <- opt$coordinateFile
rLibs             <- opt$rLibraries
...
get.partially.matching.reads <- function(workDir, regionDir, BAMDir, BAMfile){
  ...
  cmd <- paste(SAMTOOLS, ' view -f 0x2 ', BAMDir, '/', BAMfile, ' | grep -F -f ', kmerFile, ' >> ', regionDir, '/fished.sam', sep = '')
  system(cmd)

  # convert to fastq
  ## Edited by: SS On:10072021 ## "PicardException: Illegal mate state" generates truncated fastq files. Bedtools ignores reads without mate read
  ## mapped in the bam file and  generates fastq.1 and fastq.2 for the rest of the properly paired reads.

  # cmd <- paste('gatk ', 'SamToFastq -I ', regionDir, '/fished.sam -F ', regionDir, '/fished.1.fastq -F2 ', regionDir, '/fished.2.fastq -VALIDATION_STRINGENCY SILENT', sep = '')
  cmd <- paste(SAMTOOLS, ' sort -n -o ', regionDir, '/fished.sorted.sam ', regionDir, '/fished.sam', sep="")
  write.table(cmd, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
  system(cmd)
  cmd <- paste(BEDTOOLS, ' bamtofastq -i ', regionDir, '/fished.sorted.sam -fq ', regionDir, '/fished.1.fastq -fq2 ', regionDir, '/fished.2.fastq', sep='')
  write.table(cmd, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
  system(cmd)
...
combine.fastqs <- function(chr6.f1, chr6.f2, fished.f1, fished.f2){
  print(paste(chr6.f1,chr6.f2,fished.f1,fished.f2))
  ## Edited by: SS-04_with_trycatch On: 06082021: Error handeling if one of the inputs are empty
  fished.f1.seq <- data.frame("V1"=c())
  fished.f2.seq <- data.frame("V1"=c())
  fished.f1.seq.toadd <- list()
  fished.f2.seq.toadd <- list()
  tryCatch(
  {  
	...
	error=function(cond) {
	print("chr6.f1.seq or fished.f1.seq.toadd is empty")	    
  })
...
count.events <- function(BAMfile, n, bamreads){
  if(as.numeric(bamreads) > 0){
...
params <- list(full.patient, workDir, hlaPath, normalBAMfile, BAMDir, HLAfastaLoc, CopyNumLoc, NOVODir, GATK, SAMTOOLS, BEDTOOLS, JELLYFISH)
names(params) <- c('full.patient', 'workDir', 'hlaPath', 'normalBAMfile', 'BAMDir', 'HLAfastaLoc', 'CopyNumLoc', 'NOVODir', 'GATK', 'SAMTOOLS', 'BEDTOOLS', 'JELLYFISH')
document.params(params, log.name)

## Edited by: SS On:04082020
#############################
# global variables 			# 
#############################
## To keep track of all the alleles for which prediction fails due to no read coverage in input BAM or reads don't pass mismatch/even (indels,SNV) filters.
skipAlleleList <- c()
...
## Edited by: SS On:11022021: Added trycatch and Simplified the snippet.
tryCatch(
  {
  ...
  error = function(cond){
    message(cond)
    stop("Invalid patient HLA input. The doesn't exists or its empty.")
  }
)

hlaFasta   <- seqinr::read.fasta(HLAfastaLoc)
...
suffix = sapply(hlaAlleles,function(x){ return(paste(unlist(strsplit(x,"_"))[1:2],collapse="_"))})
suffixTable = data.frame(table(suffix))
print(suffixTable)
homoAllele=suffixTable[which(suffixTable$Freq==1),"suffix"]
if(length(as.character(homoAllele)) > 0){
	toRemovehomoAllele <- grep(paste0(as.character(homoAllele), collapse = "|"), hlaAlleles)
	hlaAlleles <- hlaAlleles[-(toRemovehomoAllele)]
}
print(homoAllele)
write.table(paste('\nHomozygous for',paste0(homoAllele, collapse = "|"),'-- not going to see any LOH here.', '\n', sep = ''), file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
...
###############################
  ...
  for(BAMfile in BAMfiles){
    ...
    ## Edited by: SS On:04142020
    ## Grab coordinates from a file instead of hardcoding within this script. This enable to look for LOH any where in the genome.
    coordinateList <- paste(" ",read.table(coordinateFile)[,2,drop=TRUE]," >> ")

    sapply(coordinateList, function(x){
      samtoolsCMD <- paste(SAMTOOLS," view -f 0x2 ", BAMDir, '/', BAMfile, x,regionDir,"/",BAMid,".chr6region.sam",sep="")
      write.table(samtoolsCMD, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
      system(samtoolsCMD)
    })
    ...
    ## Edited by: SS On:10072021 ## "PicardException: Illegal mate state" generates truncated fastq files. Bedtools ignores reads without mate read
    ## mapped in the bam file and  generates fastq.1 and fastq.2 for the rest of the properly paired reads.
    # samToFastQ <- paste("gatk ","SamToFastq ","-I ",regionDir,"/",BAMid,".chr6region.sam"," -F ",regionDir,"/",BAMid,".chr6region.1.fastq"," -F2 ",regionDir,"/",BAMid,".chr6region.2.fastq"," -VALIDATION_STRINGENCY SILENT",sep="")
    cmd <- paste(SAMTOOLS,' sort -n -o ', regionDir, "/", BAMid, ".chr6region.sorted.sam ", regionDir, "/", BAMid, ".chr6region.sam", sep="")
    write.table(cmd, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
    system(cmd)
    samToFastQ <- paste(BEDTOOLS," bamtofastq -i ", regionDir, "/", BAMid, ".chr6region.sorted.sam", " -fq ", regionDir, "/", BAMid, ".chr6region.1.fastq ", " -fq2 ", regionDir, "/", BAMid, ".chr6region.2.fastq", sep='')
    ...
    # sort
    sortBAM <- paste(GATK," SortSam"," -I ",regionDir, '/', BAMid, '.chr6region.patient.reference.hlas.bam'," ","-O ",regionDir,"/",BAMid,".chr6region.patient.reference.hlas.csorted.bam", " -SORT_ORDER coordinate --VALIDATION_STRINGENCY SILENT ",sep="")
    write.table(sortBAM, file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
    system(sortBAM)
    
    # remove duplicates
    removeDup <- paste(GATK,' MarkDuplicates -I ', regionDir, '/', BAMid, '.chr6region.patient.reference.hlas.csorted.bam -O ', regionDir, '/', BAMid, '.chr6region.patient.reference.hlas.csorted.noduplicates.bam --REMOVE_SEQUENCING_DUPLICATES TRUE --VALIDATION_STRINGENCY SILENT -M marked_dup_metrics.txt', sep = '')
    ...
    {
      ## Edited by: SS On:04082020
      print(paste("Making bam for allele",allele))
      bam = paste(regionDir,"/",BAMid,".type.",allele, ".bam",sep="")
      bai = paste(regionDir,"/",BAMid,".type.",allele, ".bam.bai",sep="")
      ...
      ## Edited by: SS On:04082020
      ## Filter out reads that have too many events -- has to be done here because some reads map to multiple alleles
      ## Check if there are any reads aligned to the HLA allele in the bam. If There are no reads then skip creating filtered.bam for this allele.
      totalreads <- countBam(bam, index=bai, param=ScanBamParam())[[6]]
      passed.reads <- count.events(paste(regionDir, '/', BAMid, '.type.', allele, '.bam', sep = ''), n = numMisMatch, bamreads = totalreads)
      ...
      extractCMD <- paste(GATK," FilterSamReads ", "-I ",  regionDir, '/', BAMid, '.type.', allele, '.bam' , " -FILTER includeReadList -READ_LIST_FILE ", regionDir, "/", BAMid, '.', allele, ".passed.reads.txt", ' -O ', regionDir, '/', BAMid, '.type.', allele, '.filtered.bam', sep = '')
      ...
      if(length(passed.reads) == 0){
        name <- allele
        value <- paste("For sample",basename(bam),"total reads in BAM for",allele,"are ",totalreads,"Reads passed mismatch+indel filter are ",length(passed.reads),sep="\t")
        skipAlleleList <- c(skipAlleleList, setNames(value,name))
        print(paste("*****************Reads didnt pass filter for allele ***********************",allele))
      }
...
###################################################################
# Using mpileup get coverage of each allele in every bams file    # 
###################################################################
...
if(runWithNormal){
  ...
  print(paste('Total Unique mapped reads using samtools flagstat: ',names(regionUniqMappedRegions)))
  #kv
  print(paste('KV - Normal name:'))
  print(paste(normalName))
  #kv
...
PatientOutPut <- data.frame()
regionSpecOutPut <- data.frame()
...
for (region in regions)
{
  ...
  {  
    ...
    ## Set variables . This is needed when none of the alleles pass the loop below. 
    {
      HLAtype1Log2MedianCoverage <- NA
      HLAtype2Log2MedianCoverage <- NA
      HLAtype1Log2MedianCoverageAtSites <- NA
      HLAtype2Log2MedianCoverageAtSites <- NA
      PVal                              <- NA
      UnPairedPval                      <- NA
      PVal_unique                       <- NA
      UnPairedPval_unique               <- NA
      LossAllele                 <- NA
      KeptAllele                 <- NA
      numMisMatchSitesCov        <- NA
      tmp                        <- NA
      propSupportiveSites        <- NA
      ...
      Total_Diff_Pos_between_alleles       <- NA
      Total_Mismatch_Pos_between_alleles   <- NA
      Total_GAP_Pos_between_alleles        <- NA
      Pass_HLA_Allele1_Diff_pos_Normal     <- NA
      Pass_HLA_Allele2_Diff_pos_Normal     <- NA

      HLALogR_Tumor_vs_Normal              <- NA
      Allele_Freq_Observed_HLA_type1       <- NA
      Allele_Freq_expected_HLA_type1       <- NA
      minCoverageFilter_gene               <- minCoverageFilter
    }

    ## Edited by: SS On:04072020
    HLA_class_I_II_genes <- unique(sapply(hlaAlleles, function(x){ return(paste(unlist(strsplit(x, '_'))[1:2],collapse = "_"))}))
    for (HLA_gene in HLA_class_I_II_genes){
      ## Set variables . This is needed to reset variables for each allele 
      print(paste("In Coverage Step Line 869 ",HLA_gene))
      {
        HLAtype1Log2MedianCoverage <- NA
        HLAtype2Log2MedianCoverage <- NA
        HLAtype1Log2MedianCoverageAtSites <- NA
        ...
        ## Edited by SS: 07/18/2022: Moved the following from the parent loop
        tmpOut_cn      <- data.frame()
      ...
      print(paste("HLA_type1 ",HLA_A_type1, "HLA_type2 ",HLA_A_type2))

      ## Edited by: SS On:04072020/07162021
      ## Check if mpileup file exists. mpileup doesn't exists in cases where given bam file doesnt have reads aligned to that HLA gene/allele.
      ## If hla mpileup file not found then skip the loop and go to next hla allele.
      checkFiles <- unlist(sapply(c(".normal.mpileup", ".tumor.mpileup"), function(x) { 
        type1 <- grepl(pattern = HLA_A_type1, x = list.files(workDir, pattern = paste0(HLA_A_type1,x), full.names = TRUE))
        type2 <- grepl(pattern = HLA_A_type2, x = list.files(workDir, pattern = paste0(HLA_A_type2,x), full.names = TRUE))
        return(list(type1,type2))
      }))
      ## Check if mpileupfile for the current allele and PBL is absent due to no read alignments.
      if(!all(checkFiles) | length(checkFiles) < 4){
        print(paste("Coverage: Skipping ",HLA_A_type1, " ", HLA_A_type2))
        next
      } else {
        #Check if normal mpileup file isnt empty. If not move to next allele.
        HLA_A_type1_Normal_cov=length(readLines(list.files(workDir, pattern = paste0(HLA_A_type1,".normal.mpileup"), full.names = TRUE)))
        HLA_A_type2_Normal_cov=length(readLines(list.files(workDir, pattern = paste0(HLA_A_type2,".normal.mpileup"), full.names = TRUE)))
        if ( HLA_A_type1_Normal_cov == 0 | HLA_A_type2_Normal_cov == 0 ){
          value <- paste0("Mpileup for allele ",HLA_A_type1," or ",HLA_A_type2,"in PBL is empty.This may be due to lack of coverage of this allele in PBL.")
          name <- paste0(c(HLA_A_type1,HLA_A_type2), collapse = "|")
          skipAlleleList <- c(skipAlleleList, setNames(value, name))
          next
        }
      }
      ...
      #perform local pairwise alignement
      {
        ...
        if(length(missMatchPositions$diffSeq1) == 0){
          skipAlleleList <- c(skipAlleleList, setNames("Alleles have no difference in dna sequence.", HLA_gene))
          next
        }
        if(length(missMatchPositions$diffSeq1) < 5){
          ...
          skipAlleleList <- c(skipAlleleList, setNames(msg, HLA_gene))
          write.table(paste('\n', msg, '\n', sep = ''), file = log.name, quote = FALSE, row.names = FALSE, col.names = FALSE, append = TRUE)
          warning(msg)
        }
      }

      ## Edited by: SS On:11162021
      ## If the Normal read coverage at diff base positions is below "minCoverageFilter", then the code throws error. To resuce such alleles, use a loop to 
      ## reduce "minCoverageFilter" by 5 in each iteration, and stop when 3 diff base positions with atleast 5 read coverage is found. 
      ## If none of the diff positions from either of the alleles have enough (5 reads) coverage then jump to next allele
      for(i in seq(from = minCoverageFilter, to = 5, by = -5)){
          minCoverageFilter_gene = i
          print(paste("This is minCoverageFilter_gene= ", i))
        ...
        #HLAtype 1 coverage
        {
          ...
          if(!runWithNormal){
            HLA_A_type1normal <- data.frame(cbind(HLA_A_type1, 1:length(HLA_type1Fasta), toupper(HLA_type1Fasta), minCoverageFilter_gene+1), stringsAsFactors=FALSE)
          ...
          HLA_A_type1normal <- HLA_A_type1normal[HLA_A_type1normal$V4>=minCoverageFilter_gene,,drop=FALSE]
        ...
        #HLAtype2 coverage
        {
          ...
          if(runWithNormal){
          ## Edited by SS: On 06082021 
            HLA_A_type2normal <- read.table(HLA_A_type2normalLoc ,sep="\t",stringsAsFactors=FALSE,quote="",fill=TRUE, row.names=NULL)
          }
          if(!runWithNormal){
          ...
          ## Edited by SS: On 06252021
          HLA_A_type2tumor  <- tryCatch(
            {
              read.table(paste(workDir, "/",region,".",HLA_A_type2,".","tumor.mpileup",sep=""),sep="\t",stringsAsFactors=FALSE,quote="",fill=TRUE, row.names=NULL)
            },
            error=function(x){
              message(paste("No reads in ", workDir, "/",region,".",HLA_A_type2,".","tumor.mpileup",sep=""))
              data.frame("V2")
            })
          ...
          #apply minimum coverage thresholds (we only apply this to the normal for now)
          HLA_A_type2normal <- HLA_A_type2normal[HLA_A_type2normal$V4>=minCoverageFilter_gene,,drop=FALSE]
          ...
          #rep(0,max(HLA_A_type2normal$V2,HLA_A_type2tumor$V2))
          names(HLA_A_type2normalCov) <- HLA_A_type2normal$V2
          #1:length(HLA_A_type2tumorCov)
      ...
      Pass_HLA_Allele1_Diff_pos_Normal <- length(names(HLA_A_type1normalCov)[names(HLA_A_type1normalCov) %in% as.character(missMatchPositions$diffSeq1)])
      Pass_HLA_Allele2_Diff_pos_Normal <- length(names(HLA_A_type2normalCov)[names(HLA_A_type2normalCov) %in% as.character(missMatchPositions$diffSeq2)])

        if( Pass_HLA_Allele1_Diff_pos_Normal >= 5 | Pass_HLA_Allele2_Diff_pos_Normal >= 5 ){
            break;
        }
      ...
      if(Pass_HLA_Allele1_Diff_pos_Normal == 0 | Pass_HLA_Allele2_Diff_pos_Normal == 0){
      ...
      if(Pass_HLA_Allele1_Diff_pos_Normal / Pass_HLA_Allele2_Diff_pos_Normal < 0.05){
      ...
      if(Pass_HLA_Allele2_Diff_pos_Normal / Pass_HLA_Allele1_Diff_pos_Normal < 0.05){
      ...
      if(extractNONmismatchReads == TRUE){
        ...
        if(!runWithNormal){
          HLA_type1normal_nomissmatch <-  data.frame(cbind(HLA_A_type1, 1:length(HLA_type1Fasta), toupper(HLA_type1Fasta), minCoverageFilter_gene+1), stringsAsFactors=FALSE)
          HLA_type1normal_nomissmatch$V4 <- as.numeric(HLA_type1normal_nomissmatch$V4)
          HLA_type2normal_nomissmatch <-  data.frame(cbind(HLA_A_type2, 1:length(HLA_type2Fasta), toupper(HLA_type2Fasta), minCoverageFilter_gene+1), stringsAsFactors=FALSE)
        ...
        if(is.null(HLA_type1tumor_nomissmatch) | is.null(HLA_type2tumor_nomissmatch) | is.null(HLA_type1normal_nomissmatch) | is.null(HLA_type2normal_nomissmatch)){
          ...
          #apply minimum coverage thresholds (we only apply this to the normal for now)
          HLA_type1normal_nomissmatch <- HLA_type1normal_nomissmatch[HLA_type1normal_nomissmatch$V4>minCoverageFilter_gene,,drop=FALSE]
          HLA_type2normal_nomissmatch <- HLA_type2normal_nomissmatch[HLA_type2normal_nomissmatch$V4>minCoverageFilter_gene,,drop=FALSE]
      ...
      # infer copy number using combined BAF and logR
      {
        ...
        print(paste("0a. In Coverage Step Line 1337 "))
        print(missMatchseq1)
        print(paste("0b. In Coverage Step Line 1337 "))
        print(missMatchseq2)
        ...
        ## Edited by SS: 07012023. In cases when the difference in the length of HLA-I and HLA-II is larger, NA are 
        ## substituted by 0 .
        binLogR[is.na(binLogR)] <- 0
      ...
      print(paste("1. In Coverage Step Line 1287 "))
      print(tmpOut_cn)
      ...
      print(paste("2. In Coverage Step Line 1287 "))
      print(tmpOut_cn)
      ...
      median_expectedBAF <- median(combinedTable$expectedBAF, na.rm = TRUE)
      median_BAFcombined  <- median(combinedTable$BAFcombined, na.rm = TRUE)
      median_logRcombined <- median(combinedTable$logRcombined, na.rm = TRUE)

      print(paste("3. In Coverage Step Line 1412 "))
      print(combinedTable)

      print(paste("4. In Coverage Step Line 1517 "))
      # t tests of coverage
      misMatchCoveredInBoth <- cbind(ifelse(missMatchPositions$diffSeq1 %in% names(HLA_A_type1normalCov),1,0), 
                                            ifelse(missMatchPositions$diffSeq2 %in% names(HLA_A_type2normalCov),1,0))
      missMatchseq1 <- missMatchPositions$diffSeq1[rowSums(misMatchCoveredInBoth)==2]
      missMatchseq2 <- missMatchPositions$diffSeq2[rowSums(misMatchCoveredInBoth)==2]
      
      print(paste("5. In Coverage Step Line 1524 "))
      ...
      if(nrow(tmpOut) > 1){
	        ...
	        error=function(cond){
		          print(paste("Caught the error ",cond))
      ...
      print(paste("6. In Coverage Step Line 1558 "))
     ...
     #let's put togehter the output
      ...
      tmp                        <- length(which(sort(log(c(HLA_A_type1tumorCov/HLA_A_type1normalCov),2)[as.character(missMatchseq1)]-
                                                              log(c(HLA_A_type2tumorCov/HLA_A_type2normalCov),2)[as.character(missMatchseq2)])>0))/length(missMatchseq2)
      ...
      if(performIntegerCopyNum)
      {
        ...
        Total_Diff_Pos_between_alleles <- length(missMatchPositions$diffSeq1)
        Total_Mismatch_Pos_between_alleles <- paste(length(which(missMatchPositions$diffType1==1)),";",length(which(missMatchPositions$diffType2==1)))
        Total_GAP_Pos_between_alleles <- paste(length(which(missMatchPositions$diffType1==2)),";",length(which(missMatchPositions$diffType2==2)))

        Allele_Freq_expected_HLA_type1       <- median_expectedBAF
        Allele_Freq_Observed_HLA_type1       <- median_BAFcombined
        HLALogR_Tumor_vs_Normal              <- median_logRcombined
        minCoverageFilter_gene               <- minCoverageFilter_gene
      }
      
      print(paste("7. allValues In Coverage Step Line 1652 "))
      out <- cbind(region,HLA_A_type1,HLA_A_type2
                   ,Allele_Freq_expected_HLA_type1, Allele_Freq_Observed_HLA_type1,HLALogR_Tumor_vs_Normal
                   ,HLAtype1Log2MedianCoverage,HLAtype2Log2MedianCoverage,HLAtype1Log2MedianCoverageAtSites,HLAtype2Log2MedianCoverageAtSites
                   ...
                   ,Total_Diff_Pos_between_alleles
                   ,Total_Mismatch_Pos_between_alleles
                   ,Total_GAP_Pos_between_alleles
                   ,Pass_HLA_Allele1_Diff_pos_Normal
                   ,Pass_HLA_Allele2_Diff_pos_Normal
                   ,minCoverageFilter_gene )

      ## Edited by SS:07/18/2022: Edited multiple lines to save the complete output into single dataframe.
      #combinedTable$region <- region
      #combinedTable$HLA_A_type1 <- HLA_A_type1
      #combinedTable$HLA_A_type2 <- HLA_A_type2

      #HLAoutPut <- cbind(combinedTable,out)

      print(paste("4. In Coverage Step Line 1600 "))
      #print(HLAoutPut)

      
      #PatientOutPut    <- rbind(PatientOutPut,HLAoutPut)
      PatientOutPut    <- rbind(PatientOutPut, out)
      regionSpecOutPut <- PatientOutPut

      #HLABAFsummaryLoc <- paste(workDir, '/', region, '.', HLA_gene, ".",minCoverageFilter,".DNA.IntegerCPN_CI.xls",sep="")
  ...
  if(plottingStep)
  {
   ...
   #pdf(paste(figureDir, '/', region,".minCoverage_",minCoverageFilter,".HLA.pdf",sep=""),width=10,height=6)
    pdf(paste(figureDir, '/', region,".HLA.pdf",sep=""),width=10,height=6)
    ## Edited by: SS On:04072020
    HLA_class_I_II_genes <- unique(sapply(hlaAlleles, function(x){ return(paste(unlist(strsplit(x, '_'))[1:2],collapse = "_"))}))
    for (HLA_gene in HLA_class_I_II_genes)
    {
      ...
      ## Edited by: SS On:04082020
      ## Fetch the HLA alleles for the genes. It can be simpler.
      HLA_As            <- grep(HLA_gene,hlaAlleles,value=TRUE)
      if(length(HLA_As)<=1){ next }
      HLA_A_type1 <- HLA_As[1]
      HLA_A_type2 <- HLA_As[2]
      
      ## Edited by: SS On:04072020/07162021
      ## Check if mpileup file exists. mpileup doesn't exists in cases where reads aligned to that HLA gene/allele not found in the bam file.
      ## If hla mpileup file not found then skip the loop and go to the next hla.
      checkFiles <- unlist(sapply(c(".normal.mpileup", ".tumor.mpileup"), function(x) { 
        type1 <- grepl(pattern = HLA_A_type1, x = list.files(workDir, pattern = paste0(HLA_A_type1,x), full.names = TRUE))
        type2 <- grepl(pattern = HLA_A_type2, x = list.files(workDir, pattern = paste0(HLA_A_type2,x), full.names = TRUE))
        return(list(type1,type2))
      }))
      ## Go to next HLA allele if there mpileupfile for the present one is absent due to no read alignments.
      print(paste("HLA_type1 ",HLA_A_type1, "HLA_type2 ",HLA_A_type2))
      if(!all(checkFiles) | length(checkFiles) < 4){
        print(paste("Coverage: Skipping ",HLA_A_type1, " ", HLA_A_type2))
        next
      ...
      if(runWithNormal){
        ...
        # PLOT 4 ## tumor and normal coverage for allele 1
      ...
      if(nrow(tmpOut) > 0){
        if(runWithNormal){
          boxplot(tmpOut[,2],tmpOut[,4],col=c('#de2d2699','#3182bd99'),boxwex=0.2,ylim=c(-2,2),names=c(HLA_A_type1,HLA_A_type2),las=1,main=paste('Paired t.test (common sites) p.val=',signif(PairedTtest$p.value,3)),ylab=('logR ratio'))
        }
        if(!runWithNormal){
          boxplot(tmpOut[,2],tmpOut[,4],col=c('#de2d2699','#3182bd99'),boxwex=0.2,names=c(HLA_A_type1,HLA_A_type2),las=1,main=paste('Paired t.test (common sites) p.val=',signif(PairedTtest$p.value,3)),ylab=('Coverage'))
        }
      ...
      if(nrow(tmpOut_unique) > 0){
        if(runWithNormal){
          boxplot(tmpOut_unique[,2],tmpOut_unique[,4],col=c('#de2d2699','#3182bd99'),boxwex=0.2,ylim=c(-10,10),names=c(HLA_A_type1,HLA_A_type2),las=1,main=paste('Paired t.test (mismatch sites) p.val=',signif(PairedTtest_unique$p.value,3)),ylab=('"logR ratio"'))
        }
        if(!runWithNormal){
          boxplot(tmpOut_unique[,2],tmpOut_unique[,4],col=c('#de2d2699','#3182bd99'),boxwex=0.2,names=c(HLA_A_type1,HLA_A_type2),las=1,main=paste('Paired t.test (mismatch sites) p.val=',signif(PairedTtest_unique$p.value,3)),ylab=('"Coverage"'))       
        }
...
# Write the information about the failed HLA alleles
skipAlleleDF <- data.frame(keyName=names(skipAlleleList), value=skipAlleleList, row.names=NULL)
write.table(skipAlleleDF,  paste0(workDir, '/',"Alleles_not_processed.txt"), sep="\t", quote = FALSE, col.names = TRUE, row.names = FALSE)

# write the output
HLAoutLoc <- paste(workDir, '/', full.patient,'.',"DNA.HLAlossPrediction_CI.txt",sep="")
